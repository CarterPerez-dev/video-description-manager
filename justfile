...
[group('db')]
migrate *ARGS:
    docker compose -f dev.compose.yml exec backend uv run alembic upgrade {{ARGS}}

[group('db')]
migration message:
    docker compose -f dev.compose.yml exec backend uv run alembic revision --autogenerate -m "{{message}}"

[group('db')]
init:
    docker compose -f dev.compose.yml exec backend uv run alembic revision --autogenerate -m "initial"
    docker compose -f dev.compose.yml exec backend uv run alembic upgrade head

[group('db')]
rollback:
    docker compose -f dev.compose.yml exec backend uv run alembic downgrade -1

[group('db')]
db-history:
    docker compose -f dev.compose.yml exec backend uv run alembic history --verbose

[group('db')]
db-current:
    docker compose -f dev.compose.yml exec backend uv run alembic current
